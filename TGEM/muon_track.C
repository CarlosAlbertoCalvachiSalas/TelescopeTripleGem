...
int main(int argc, char *argv[]) {
	...
	const double axis_x = 0.15;         // X-width of drift simulation will cover between +/- axis_x
	const double axis_y = 0.15;          // Y-width of drift simulation will cover between +/- axis_y  
	const double axis_z = 0.303;        // Z-width of drift simulation will cover between +axis_z and neg_axis_z
	...
	TCanvas *cGeom = new TCanvas("Track", "Avalancha por muon de 150 MeV que ingresa al detector");
	...
	viewDrift->SetArea(-axis_x, -axis_y, neg_axis_z, axis_x, axis_y, axis_z);
	...
	int nReadOutElectrons = 0; // Variable to save number of electrons that reach readout plane
	...
	for(int i = 0; i < events; ++i){
		...
		while(track->GetCluster(xc, yc, zc, tc, nc, ec, extra)){
			...
			for(int j = 0; j < nc; ++j) {
				...
				for(int k = nAvalanche; k>0; k--){
					...
					dataElectrons  << argon << ","  <<  intEnergy << ","  << voltage << "," << i + 1 << "," << nAvalanche - k << "," << xe2 << "," << ye2 << "," << ze2 <<  "," <<  te2 << "," <<  e2  << "\n" ;

				}

				// Print the avalanche size
				int ne, ni;
		...
		//printf("Ganancia = %f\n", double(nReadOutElectrons)/double(clusters));
		//printf("Ganancia = %f\n", double(nReadOutElectrons));
	}

	dataElectrons.close();
	dataAvalanches.close();

	//ddata_elec_read.close();

	/*
	ViewFEMesh * vFE = new ViewFEMesh();
	vFE->SetArea(-axis_x, -axis_y, neg_axis_z, axis_x, axis_y, axis_z);
	vFE->SetCanvas(cGeom);
