...
def readAvalancheData(ar, energy, V, showHists = False, showDistribution = False):
	filePath = str(V)+'/'+'dataAvalanchesAr{}E{}V{}.csv'.format(ar, energy, V)
...
def fixedE(energy = 9):
	...
	"""
	plt.title(r'Distribución electrones por evento para $\%$ Ar/CO2 fijos a ')
	plt.hist(gainAvgStd[0, :], label = r'$\mathrm{Ar/CO_2} = $' + '{}/{}'.format(60, 40), color = 'purple')
	plt.hist(gainAvgStd[1, :], label = r'$\mathrm{Ar/CO_2} = $' + '{}/{}'.format(65, 35), color = 'red')
	plt.hist(gainAvgStd[2, :], label = r'$\mathrm{Ar/CO_2} = $' + '{}/{}'.format(70, 30), color = 'green')
	plt.hist(gainAvgStd[3, :], label = r'$\mathrm{Ar/CO_2} = $' + '{}/{}'.format(75, 25), color = 'magenta')
	plt.hist(gainAvgStd[4, :], label = r'$\mathrm{Ar/CO_2} = $' + '{}/{}'.format(80, 20), color = 'blue')
	plt.ylabel('Desviación ganancia promedio')
	plt.xlabel('V')
	plt.legend()
	plt.tight_layout()
...
readAvalancheData(60, 9, 400, showHists = True, showDistribution = True)
...
for ar in argons:
	...
	for v in voltages:
		...
		muonAvalTot = dataFrame.groupby('step')['avalTot'].sum()

		gainTot, gainTotStd = muonAvalTot.mean(), muonAvalTot.std()

		errorTot = gainTotStd/np.sqrt(len(muonAvalTot))

		muonAvalFil = dataFrame.groupby('step')['filTot'].sum()

		gainFil, gainFilStd = muonAvalFil.mean(), muonAvalFil.std()

		errorFil = gainFilStd/np.sqrt(len(muonAvalFil))

		gains.append(gainFil)
		errorGains.append(errorFil)
		#print(gainTot, errorTot)

	plt.errorbar(voltages, gains, yerr=errorGains, fmt='.k')
	plt.plot(voltages, gains, label = 'Ar/CO2 = {}/{}'.format(ar, 100 - ar))
